...
COPY LitterRobot/*.csproj ./LitterRobot/
RUN mkdir -p vendor && dotnet restore LitterRobot
COPY . ./
# Build the app
# Find the non-test csproj file, build that project
ARG BUILD_VERSION=0.0.0.0
RUN dotnet build -o output -c Release --no-restore -p:Version=$BUILD_VERSION LitterRobot

FROM mcr.microsoft.com/dotnet/runtime:6.0 AS runtime
COPY --from=build /src/output app
ENTRYPOINT ["dotnet", "./app/LitterRobot.dll"]
